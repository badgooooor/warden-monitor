...
import WardenPair from "../abi/WardenPair.json";
...
export class TokenHelper {
  ...
  async getTokenSymbol(address: string) {
    const tokenContract = this.web3Service.getContract(BEP20, address);
  ...
  async getTokenPair(lpAddress: string) {
    const lpContract = this.web3Service.getContract(WardenPair, lpAddress);
    const token0Address = (
